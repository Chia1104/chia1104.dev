...
<Image
    img_src="/posts/React hooks 在use什麼(Part 1)/react-hook.png"
    img_alt="React hooks 在use什麼(Part 1)"
/>
...
<Quote
    type={'info'}
>
    - 我們可以使用 `useEffect` 來防止 infinite loop。
</Quote>
...
<Quote
    type={'warning'}
>
    - 我們一定要在 `onClick` 中設置一個 callBack function，不然會觸發 infinite loop。
    - 這樣 React 才會知道 `setCount` 是渲染完才接受的 function。
</Quote>
