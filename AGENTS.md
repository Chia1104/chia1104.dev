...
**Config injection.** `packages/api` parses no env of its own. The hosting app calls `configureORPC({ rateLimit, projectId, aiAuthPrivateKey })` once at boot (`apps/service/src/factories/orpc.factory.ts`). Domain ports travel on the oRPC context (`BaseOSContext` in `packages/api/orpc/utils.ts`): `hooks.onFeedChanged` / `onFeedRemoved`, `indexing`, `agentKinds`. `createORPCContext` in `orpc.factory.ts` is the one place `apps/service` supplies them; a process that leaves one out (the dashboard's in-process router client) gets `SERVICE_UNAVAILABLE` from the routes that need it. Anything needing a long-lived process, a DB handle, or gateway credentials belongs in the app, not in `packages/api`.
