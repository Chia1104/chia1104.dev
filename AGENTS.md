...
- CI selects work from Turbo's dependency graph with `--affected`; do not add path filters or per-app workflows. A workspace with a `Dockerfile` gets an image build.
