...
export const tags = pgTable(
  ...
  (table) => [uniqueIndex("tag_slug_index").on(table.slug)]
);
...
export const tagTranslations = pgTable(
  ...
  (table) => [
    uniqueIndex("tag_translation_tag_locale_index").on(
      table.tagId,
      table.locale
    ),
    index("tag_translation_tag_id_index").on(table.tagId),
    index("tag_translation_locale_index").on(table.locale),
    index("tag_translation_name_index").on(table.name),
  ]
);
...
export const assets = pgTable(
  ...
  (table) => [
    index("asset_user_id_index").on(table.userId),
    index("asset_name_index").on(table.name),
  ]
);
...
const feedsExtraConfig = (
  ...
  index("feed_user_id_index").on(table.userId),
  index("feed_type_index").on(table.type),
  index("feed_published_index").on(table.published),
  index("feed_default_locale_index").on(table.defaultLocale),
];
...
export const feedTranslations = pgTable(
  ...
  (table) => [
    uniqueIndex("feed_translation_feed_locale_index").on(
      table.feedId,
      table.locale
    ),
    index("feed_translation_feed_id_index").on(table.feedId),
    index("feed_translation_locale_index").on(table.locale),
    index("feed_translation_title_index").on(table.title),
    index("feed_translation_embedding_index").using(
      "hnsw",
...
export const contents = pgTable(
  ...
  (table) => [
    uniqueIndex("content_feed_translation_id_index").on(
...
export const assetsToTags = pgTable(
  ...
  (t) => [
    ...
    index("assets_to_tags_asset_id_index").on(t.assetId),
    index("assets_to_tags_tag_id_index").on(t.tagId),
  ]
);
...
export const feedsToTags = pgTable(
  ...
  (t) => [
    ...
    index("feeds_to_tags_feed_id_index").on(t.feedId),
    index("feeds_to_tags_tag_id_index").on(t.tagId),
  ]
);
