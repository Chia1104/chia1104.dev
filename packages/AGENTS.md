...
- `feed_draft` is the only write path for post content shared by the editor, MCP and the writing agent; `api/feeds/draft` owns open, patch, edit, apply, discard and restore; an edit is an exact-string replacement matched under the draft lock, so it needs no revision to be safe. `feed` changes only through apply or the feed-level `feeds.update`, and only those start indexing. A writing session is not bound to a draft: `agent.writing_session_draft` records the drafts it worked on, and a prompt hands one over as an attachment that the kind's `attach` admits before the turn's run starts.
- Repositories name the acting user `userId` and answer `not_found` for a row another user owns, checked on the locked row; services pass the configured `adminId` into that parameter rather than reading first to check ownership.
...
`@chia/contents/mdx-components` is the only list of what post MDX can render; the writing agent's `mdx-components` skill in `agent-writing/src/prompts/skills.ts` restates it for the model, so a component added or removed on one side changes the other in the same commit.
