...
- **`agent-public`** — the public reader: the prompt, the `read`-only policy, the tight turn budget and the cheap house model allowlist (`HOUSE_PUBLIC_MODEL_IDS`) over `agent-content`'s tools; no tools, ports or state of its own. Native BYOK providers are open because the visitor pays for them.
