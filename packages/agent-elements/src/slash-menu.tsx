...
export const SlashMenu = ({
  ...
  // The textarea owns focus for continued typing and IME; scrolling follows visual focus instead.
  useLayoutEffect(() => {
    const option = activeOptionRef.current;
    option?.scrollIntoView({ block: "nearest", inline: "nearest" });
    onActiveDescendantChange(option?.id);
  ...
  return (
    <div className="bg-surface/70 border-border absolute inset-x-0 bottom-[calc(100%+0.5rem)] z-40 w-full max-w-[95%] justify-self-center overflow-hidden rounded-2xl border p-1.5 shadow-lg backdrop-blur-xl">
      <ListBox
        ...
        renderEmptyState={() => (
          <p className="text-muted px-3 py-8 text-center text-xs">
            {emptyText}
          </p>
        )}>
        {items.map((item) => (
