...
import type { UserMessage } from "./messages.ts";
...
import { bindEngineTools } from "./turn/engine-tools.ts";
import {
  callsOf,
  openCallsOf,
  resumeItemOf,
  settledDecision,
  triageInterrupt,
} from "./turn/interrupt.ts";
import { runPass } from "./turn/pass.ts";
import type { PassContext, PassResult, TurnCalls } from "./turn/pass.ts";
...
const compactAfterTurn = async ({
  ...
  try {
    ...
    const compacted = await compactSessionIfNeeded(
      { session, binding: summariser, onUsage },
...
const executeTurn = async (
  ...
  try {
    ...
    const calls: TurnCalls = {
      ...
      declined: new Map(
        (resume?.decisions ?? [])
          .filter((decision) => !decision.approved && !decision.refused)
          .map((decision): [string, { comment?: string }] => [
            decision.toolCallId,
    ...
    const pass: PassContext = {
      ...
      volatileContext,
      policy,
      tools: bindEngineTools(tools, {
        policy,
        settings,
        log: { sessionId: agentSessionId, runId: agentRunId },
      }),
      check,
      calls,
      control,
    ...
    let result: PassResult = { status: "done" };
    ...
    while (!control.failure && !control.aborted && !transcript.failed) {
      const branch = await session.getBranch(transcript.leafId);
      for (const [toolCallId, call] of callsOf(branch)) {
        calls.names.set(toolCallId, call.name);
      }
      const runId = passes === 0 ? agentRunId : `${agentRunId}:${passes}`;
      result = await runPass(pass, {
        messages: buildBranchContext(branch, {
          api: binding.api,
          pendingToolCallIds: resumeItems ? openCallsOf(branch) : undefined,
        }),
        runId,
        parentRunId,
        resume: resumeItems,
      ...
      // The engine stopped on gated calls; the turn answers what it can before asking.
      const gated = callsOf(await session.getBranch(transcript.leafId));
      let batch: ApprovalBatch;
      try {
        batch = await triageInterrupt(
          result.toolCallIds.map((toolCallId) => {
            const call = gated.get(toolCallId);
            return {
              toolCallId,
              toolName: call?.name ?? calls.names.get(toolCallId) ?? "unknown",
      ...
      for (const call of [...batch.requests, ...batch.settled]) {
        calls.checked.add(call.toolCallId);
      }
      if (control.failure) break;
      if (batch.requests.length > 0) {
        approvals = batch;
        break;
      }
      // An interrupt with no gated call to answer would stop the next pass the same way.
      if (batch.settled.length === 0) {
        control.fail({
          kind: AgentErrorKind.Internal,
      ...
      for (const call of batch.settled) {
        if (call.reason !== undefined) {
          calls.refused.set(call.toolCallId, call.reason);
        }
      }
      parentRunId = runId;
      resumeItems = batch.settled.map((call) =>
        resumeItemOf(settledDecision(call))
      );
      passes += 1;
    }
    ...
    let failure =
      control.failure ??
      (result.status === "error" ? result.failure : undefined);
    ...
    const stopped =
      !failure && (control.aborted || result.status === "aborted");
