 ...
 * arguments are validated against before `execute` sees them. The runtime binds them to the
 * engine for a turn.
 */
...
> extends ToolSpec<TParameters> {
  /**
   * Receives arguments the engine has already validated against `parameters`. A method, so a
   * tool typed for its own parameters still fits a kind's `AgentTool[]`.
   */
