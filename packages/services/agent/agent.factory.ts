...
import type { AgentAdminService } from "./admin";
import type { AgentKindService } from "./agent.service";
import type { AgentUsageService } from "./usage";
...
export const createAgentFactory = (
  ...
  return {
    ...
    async create(kind) {
      ...
      const { createAgentKindService } = await import("./kind.service");
      return createAgentKindService(definition, options);
    },
    async createAdmin() {
      const { createAgentAdminService } = await import("./admin");
      return createAgentAdminService({ kinds, load });
    },
    async createUsage() {
      const { createAgentUsageService } = await import("./usage");
