 ...
 * active lesson is a standing instruction. Writes go through `memory/write.service.ts` so the
...
export const memoryRouter = contractOS.memory.router({
  list: listMemoriesRoute,
  get: getMemoryRoute,
  update: updateMemoryRoute,
  remove: removeMemoryRoute,
  "lesson:approve": approveLessonRoute,
  consolidate: consolidateMemoryRoute,
});
