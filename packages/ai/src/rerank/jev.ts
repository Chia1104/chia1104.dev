...
/**
 * One call: a `choice` over the candidates and a `noul` on whether any of
 ...
 * probabilities always sum to 1, so the noul is what tells a genuine answer
  ...
  }).systemOne(
    {
      ...
      questions: {
        best: choice(
          ...
          Object.fromEntries(entries.map((entry) => [entry.id, null]))
        ),
        answerable: noul(
