...
const RepoItem: FC<Props> = ({repo}) => {
    return (
        <span className="c-button-secondary transform group-hover:-translate-x-1 group-hover:-translate-y-1 w-full h-full flex flex-col">
            <header className="text-2xl line-clamp-1 mb-2 group-hover:c-text-green-to-purple ">{repo.node.name}</header>
            <p className="c-description text-base line-clamp-2">{repo.node.description}</p>
                ...
                <span className="mr-5 flex items-center">
                    {
                        repo.node.stargazerCount > 0 ?
                            ...
                            </svg> : <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5 mr-1" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
...
export default memo(RepoItem, (prev, next) => prev.repo.node.id === next.repo.node.id)
