...
- `agents/` — the agent kinds this process serves. `registry.ts` (`AGENT_KINDS`, `agentKinds`, `loadAgentKind`) is the one place a kind is registered and is boot-safe: it restates each kind's `minTier` for the guards and loads the definition with a dynamic import. `service.ts` is the generic `AgentKindService` over a definition, `writing.ts` binds the writing kind without an execution host (this process seeds drafts and serves the session API, it never runs a turn), `public.ts` does the same for the public kind, and `admin.ts` is the `AgentAdminService` behind the dashboard's agent workspace. The kind contract, task registry and config resolution live in `@chia/agent-host` so `apps/workflow` shares them. A new kind is a sibling of `writing.ts` here and in `apps/workflow/src/agents/`, plus one registry entry; see [`docs/agent-architecture.md`](../docs/agent-architecture.md) §2 and §13.
...
- `agents/` — `registry.ts` (`loadAgentKind`), `writing.ts`, the writing kind bound to its execution host: content port (`services/agent-content.port.ts`), web port (`agent-web.port.ts`), memory port (`agent-memory.port.ts`), credentials, and memory consolidation through `workflowControl`; and `public.ts`, the public kind bound to a `public`-visibility `ContentReadPort` and nothing else.
