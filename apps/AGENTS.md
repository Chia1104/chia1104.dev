...
The post editor edits `feed_draft`, never the feed: autosave is a compare-and-set on the draft revision, changes from the agent, MCP or another tab arrive over `feeds.draft:watch` as invalidations of the current draft query. Watch connections and database-listener reconnects resynchronize current state without replaying revision history; there is no client polling. The mounted agent session additionally reports its draft-tier tool calls through `onToolEvent`, so the editor can show what the agent is doing to the open draft and refresh it as soon as a call settles. Only Apply writes the feed. Feed-level switches (published, date, delete) call `feeds.update` directly.
