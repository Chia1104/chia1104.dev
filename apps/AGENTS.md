...
- Public agent chat lives in `src/components/agent/` and uses `@chia/agent-elements` content renderers only. `AppLayout` mounts `AgentContextProvider` around the page and the chat dock; a post page wraps its body in `ArticleAgentContext`, which provides the post as a `feed` attachment while mounted and files the reader's selected passage as a `selection` attachment request, opening the dock to send it. Guest sessions come from better-auth's anonymous plugin and require the site captcha before minting. `@chia/ui/captcha` is the shared challenge widget.
