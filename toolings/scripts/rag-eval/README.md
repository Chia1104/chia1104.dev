...
pnpm --filter rag-eval eval jev=true           # also hybrid+jev (see below)
```
...
- `hybrid+jev` takes hybrid's top 20 and reorders them by TypeSafe's Jev
  through the Vercel AI Gateway (`typesafe-ai/jev`), the way a reranker
  would sit between retrieval and the agent. Jev sees exactly the excerpts
  `search_memory` / `search_posts` show the agent. The mode is opt-in
  (`jev=true` or `mode=hybrid+jev`), needs `AI_GATEWAY_API_KEY` in the
  environment and bills a few thousand input tokens per query. The per-query
  cell adds Jev's probability that some candidate answers the query (`6 .93`),
  and `ans` is its mean; every golden query is answerable, so a low value is
  Jev disagreeing with the golden set.
