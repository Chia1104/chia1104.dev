...
| Phase                                            | 狀態      | 備註                                                       |
| ------------------------------------------------ | --------- | ---------------------------------------------------------- |
| Phase 1：記得住、搜得到（表 + 工具 + RAG 索引）  | ⬜ 未開始 | 完成後 agent 已可跨 session 記憶，是最小可用產品           |
...
- Schema：`packages/db/src/schemas/agent.schema.ts`（新增 `agentMemories`）、`packages/db/src/schemas/resources.schema.ts`（`resource_chunk` 加 FK + generated column 改寫）
- Repo：`packages/db/src/libs/agent/memory.ts`（新增，走既有 `./repos/agent` export）
- 共用寫入邏輯：`packages/api/memories/write.ts`（新增，仿 `feeds/write.ts`：route 與 agent port 共用「寫入 + 觸發索引」）
- RAG adapter：`packages/api/resources/agent-memory.resource.ts`（新增）+ 註冊進 `registry.ts`
- 工具：`packages/agent-writing/src/tools/memory.tool.ts`（新增）+ `registry.ts` / `tool-set.ts` / `types.ts` / `ports.ts`
- Port 實作：`apps/service/src/services/agent-memory.port.ts`（新增）；`apps/service/src/agents/writing.ts` 綁進 turn
...
- Workflow：`apps/service/src/workflows/memory-consolidation.workflow.ts` + `steps/memory-consolidation.step.ts`（新增）
...
2. **volatile context 的「本 session 已存記憶」清單（Phase 2）**——只列本 session 寫入的記憶標題，讓模型知道自己記過什麼、不重複記。一次以 `session_id` 索引的查詢，與 `draft.get` 同一風險等級（volatile 讀取失敗會讓 turn 以 `internal` 收場，這是既有的刻意設計，見 agent-architecture.md §4）。
...
`draft` tier 會發 `state:changed`，client 會多一次 draft detail refetch——無害，接受這個代價，不為此新增 tier（tier 是 policy 的詞彙表，多一個詞要有兩個以上的使用者）。`writingTurnBudget` 不動：記憶操作包含在既有的 40 次軟上限內。
...
Repo：`packages/db/src/libs/agent/memory.ts`，經 `@chia/db/repos/agent` 匯出。CRUD + `listBySession` + `listActiveLessons`。
...
BM25 索引在現有語料上重建，一次性成本，套用時注意（語料量小，秒級）。
...
仿 `feeds/write.ts` 的角色：**寫入和觸發索引永遠綁在一起**，route handler（Phase 3）和 agent port（本期）都呼叫它，索引觸發函式當明確參數收（`onMemoryChanged(id)`），不依賴 request context。實作上 `onMemoryChanged` 由 `apps/service` 供給，內容就是 start `indexResourceWorkflow({ sourceType: 'agent_memory', sourceId })`，fire-and-forget（同 `feedHooks` 語意：索引失敗不擋寫入，workflow 自己重試）。
...
`WritingToolContext`（`types.ts`）加 `memory: MemoryPort`；`RunWritingTurnOptions`（`runtime.ts`）加 `memory`；`apps/service/src/agents/writing.ts` 綁 host 實作。

`packages/agent-writing/src/tools/memory.tool.ts`：

- **`save_memory`**（draft tier）：`kind`（只開放 `fact`——`source` 是自動的、`lesson` 是 workflow 的，模型都不該手寫）、`title`、`content`（上限 4,000 字元，強制蒸餾而不是倒整頁）、`sourceUrl`（optional 但 description 強烈要求）。回傳 id 與確認訊息。
...
`tools/registry.ts` 加三個 `TOOL_NAMES` + label + tier；`tool-set.ts` 排序放在 retrieval 工具之後、draft 工具之前（read → draft 的既有敘事順序）。`summarize.ts` 若 registry 有 per-tool 摘要格式，補上對應分支。
...
- Adapter：`packages/api/resources/__tests__/agent-memory.resource.test.ts`——markdown content 的 buildChunks 形狀、archived 回 null、visibility 固定值。
- 工具：`packages/agent-writing/src/tools/__tests__/memory.tool.test.ts`，用 `InMemoryMemoryPort`（仿 `InMemoryDraftStore` 放在 domain 包內，測試與 production 走同一介面）。
...
`fetchUrlTool.execute` 成功取得頁面後，呼叫 `context.memory.save({ kind: 'source', title: page.title, sourceUrl: page.url, content: 摘錄（前 ~500 字）})`。Port 實作以 `source_url` upsert（§4.1 的 partial unique index），重訪同一頁只更新時間戳。
...
輸入從 `TurnContextInput` 加 `sessionMemories`（`runtime.ts` 的 `volatileContext` closure 裡與 `draft.get` 一起讀，`listBySession` 一次索引查詢）。空清單不輸出該段。這讓模型不重複記、且能直接引用 id 呼叫 `get_memory`。
...
Route handler 呼叫 repo 與 `memories/write.ts`，不寫 raw Drizzle（既有規範）。
...
- **`commit_draft` 核准執行成功後**：一篇文章落庫是「一個寫作任務完成」的自然邊界，這時 transcript 裡的修改往返最完整。host 的 `ContentPort.commitDraft` 實作（`apps/service/src/services/agent-content.port.ts`）成功後 fire-and-forget 啟動 workflow。
- **dash 手動觸發**（`memories.consolidate(sessionId)`，掛在 Phase 3 的 contract 上）：沒有 commit 的 session（純討論、被放棄的方向）也可能有教訓。

### 7.2 `memoryConsolidationWorkflow`

`apps/service/src/workflows/memory-consolidation.workflow.ts`，一個 step：

1. 讀 session 的 active branch（`PgSessionStorage` / entry repo 直讀，不建 `Agent`——同 maintenance 的原則）。
2. 組抽取輸入：**只有 operator 的 user 訊息與 assistant 的文字回覆，不含 tool result**（§3.6 第 1 層）。
